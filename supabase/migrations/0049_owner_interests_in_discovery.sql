-- Keşfet kartına sahip ilgi alanları teaser'ı ekleniyor (UX kalite turu).
--
-- `profiles.interests` `0041`'den beri toplanıyor ama hiçbir yerde
-- GÖSTERİLMİYORDU — yalnızca veri toplama açılmıştı, "uyum skoruna bağlama
-- ve 5. sekme bilerek kapsam dışı" notuyla (bkz. experience-roadmap.md §6).
-- Bu migration onu Keşfet kartına taşıyor: `discover_playdate_pets` ve
-- `pending_likes`'a `owner_interests` kolonu, AYNI `owner_visibility =
-- 'public'` kapısıyla (owner_display_name/owner_bio ile birebir aynı desen)
-- ekleniyor. İstemci tarafında ayrı bir sorgu YOK — 0021'deki karşılıklı
-- açıklama kuralını delmemek için mevcut satırdan geliyor.
