-- Mesafe: zorunlu eleme değil, sıralama sinyali (+ isteğe bağlı filtre)
--
-- ---------------------------------------------------------------------------
-- 1. NEDEN
--
-- `0057` arama havuzunu bölgeye kilitledi. Mesafe o günden beri havuzun
-- İÇİNDE ikinci bir sert filtre olarak duruyordu ve mahalle ölçeğinde
-- kazandırdığı hiçbir şey yoktu — buna karşılık iki gerçek durumu bozuyordu:
--
--   - **Bölgesini seçip başka şehirde olan kullanıcı.** Kadıköy'ü seçen ama
--     cihazı Ankara'da olan kişinin bütün adayları 25 km'nin dışında kalıyor;
--     destesi TAMAMEN boş açılıyordu. Bölge seçimi "kimlerle eşleşeceğim"
--     sorusunu yanıtlıyorsa, o soruya verilen cevap konumla iptal edilmemeli.
--   - **Seyahat.** Bir hafta şehir dışına çıkan kullanıcının destesi
--     yolculuk boyunca kararıyordu.
--
-- Karar: **mesafe varsayılan olarak elemez, sıralar.** Yakınlık isteyen
-- kullanıcı filtreyi kendisi açar (`distance_filter_enabled`); kapalıyken
-- bölgedeki herkes destede kalır, mesafe yalnızca sıraya etki eder. Kart
-- zaten mesafe kovasını gösteriyor, yani kullanıcı bilgisiz kalmıyor.
--
-- Varsayılan `false`: mevcut kullanıcılar dahil kimse farkında olmadan
-- eleme yaşamasın. `max_distance_km` kolonu duruyor — filtre açıldığında
-- okunan değer o.
--
-- ---------------------------------------------------------------------------
-- 2. SIRALAMA: kesin mesafe → kaba anahtarlar
--
-- `order by km` kesin bir mesafe SIRALAMASI veriyordu. Kova (`0007`) tam da
-- bunu engellemek için var: saldırgan kendi konumunu gezdirip aynı hedefin
-- sırasındaki kaymayı okuyarak kovadan daha ince bir çözünürlük elde
-- edebilirdi. Sıralama artık üç kaba anahtar:
--
--   1. mesafe kovası  ('<1' → '25+', kovası olmayan en sonda)
--   2. aktiflik kovası ('today' → 'older')
--   3. kullanıcıya ve saate bağlı sabit karıştırma
--
-- Üçüncüsü `random()` DEĞİL, bilerek: rastgele sıralamada istemci her
-- yenilemede desteyi baştan karıştırır ve kullanıcı kartların yer
-- değiştirdiğini görür. `md5(id || kullanıcı || saat)` aynı saat içinde
-- kararlı, saat başı dönüşümlü — aynı kovadaki petler zamanla sırayla öne
-- çıkar, kimse sürekli en altta kalmaz.
--
-- ---------------------------------------------------------------------------
-- 3. YOL ÜSTÜNDE: activity_bucket IMMUTABLE'dı
--
-- `0008`'de `immutable` işaretlenmiş ama gövdesi `now()` okuyor. IMMUTABLE
-- sözü "aynı girdiye her zaman aynı çıktı" demektir; planlayıcı böyle bir
-- fonksiyonu plan zamanında sabite katlayabilir ve prepared statement'lar
-- arasında taşıyabilir — yani 'today' bir gün sonra hâlâ 'today' dönebilir.
-- Bugün index ifadesinde kullanılmadığı için zarar görünmemiş. Sıralama
-- artık bu fonksiyona dayandığı için doğru volatiliteye çekiliyor.
-- ---------------------------------------------------------------------------

alter table discovery_preferences
  add column distance_filter_enabled boolean not null default false;

comment on column discovery_preferences.distance_filter_enabled is
  'Mesafe sert filtre olarak uygulansın mı. Kapalıyken max_distance_km yalnızca kullanıcının kayıtlı tercihi olarak durur, eleme yapmaz.';

-- now() okuyan bir fonksiyon immutable olamaz.
...
-- ---------------------------------------------------------------------------
-- Yazma yolu aynı kuralı söylemeli: filtre kapalıyken mesafe swipe'ı da
-- engellemez. Aksi halde destede görünen kart swipe edilemez olurdu.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Filtre yazma yolu: mesafe artık bir anahtar + değer çifti.
-- ---------------------------------------------------------------------------
...
revoke all on function update_my_discovery_filters(
  species[], integer, boolean, numeric, numeric, boolean, boolean, boolean, boolean, boolean
) from public, anon, authenticated;
